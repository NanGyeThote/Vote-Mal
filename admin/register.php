...
// Initialize a flag to indicate email error
$email_exists = false;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO admin (admin_id, username, email, password, is_approved) VALUES (?, ?, ?, ?, 0)");
        $stmt->bind_param("ssss", $admin_id, $username, $email, $password);

        // Execute statement
        if ($stmt->execute()) {
            echo "<script>document.getElementById('successModal').style.display='block';</script>";
            echo "<script>setTimeout(function() { document.getElementById('approvalModal').style.display='block'; }, 3000);</script>";
        } else {
...
<body>
    <div class="container">
        ...
        <div class="main">
            ...
            <div id="emailError" class="error-box">
            This email has already been used. Please use another email to register.
        </div>
    ...
    <div id="approvalModal" class="modal">
        <div class="modal-content">
            ...
            <p>Your registration is pending approval. Please wait for approval.</p>
        </div>
    </div>
    ...
    <script>
        // Check if email already exists (flag from PHP)
        <?php if ($email_exists): ?>
            // Show the error box for 3 seconds
