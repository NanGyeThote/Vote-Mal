...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert the new poll into the votes table
    $sql_vote = "INSERT INTO votes (title, meeting_id, description) VALUES ('$title', '$meeting_id', '$description')";
    if ($conn->query($sql_vote) === TRUE) {
        // Set the success message on successful poll creation
        $success_message = "Poll created successfully!";
    } else {
        echo "Error: " . $sql_vote . "<br>" . $conn->error;
    }
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .poll-form input[type="text"], .poll-form textarea {
            width: 100%;
            padding: 10px;
            margin-bottom: 10px;
            border-radius: 4px;
